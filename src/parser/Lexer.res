...
/// Returns a single-character string (ReScript 12: String.get returns option<string>).
let peek = (l: lexerState): option<string> => {
  if isAtEnd(l) {
    None
  } else {
    String.get(l.source, l.pos)
  }
}

/// Peek at the next character (lookahead 1).
let peekNext = (l: lexerState): option<string> => {
  if l.pos + 1 >= String.length(l.source) {
    None
  } else {
    String.get(l.source, l.pos + 1)
  }
}

/// Consume and return the current character as a single-char string.
let advance = (l: lexerState): string => {
  let ch = String.get(l.source, l.pos)->Option.getOr("")
  l.pos = l.pos + 1
  if ch == "\n" {
    l.line = l.line + 1
    l.col = 1
...
/// Check if a single-char string is alphanumeric or underscore.
let isAlphaNumeric = (ch: string): bool => {
...
let readIdentOrKeyword = (l: lexerState): token => {
  ...
  if text == "region" && !isAtEnd(l) && peek(l) == Some(".") {
    ...
    let method = String.slice(l.source, ~start=methodStart, ~end=l.pos)
    switch method {
    | "get" => RegionGet
    | "set" => RegionSet
    | "scan" => RegionScan
  ...
  if isAtEnd(l) {
    ...
    let tok = if isAlpha(ch) {
      ...
      if ch == "s" && !isAtEnd(l) {
        ...
        let name = String.slice(l.source, ~start=nameStart, ~end=l.pos)
        LifetimeName(name)
      } else {
        let nameStart = l.pos
        while !isAtEnd(l) && isAlphaNumeric(Option.getOr(peek(l), " ")) {
      ...
      | _ => Ident(ch) // Unknown character as ident
      }
    }
