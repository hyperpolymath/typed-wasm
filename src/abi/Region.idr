...
alignUp x (S a) =
  let align = S a
      remainder = modNatNZ x align SIsNonZero
  in case remainder of
       Z => x
       (S _) => x + (align `minus` remainder)
...
public export
lookupField : FieldIn name schema -> Field
lookupField (FieldHere {f}) = f
