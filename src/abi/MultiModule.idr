...
||| Proof that two field types are compatible.
||| Compatibility is strict equality — no implicit conversions.
public export
data FieldTypeCompat : FieldType -> FieldType -> Type where
  ||| Primitive types must be identical.
  PrimCompat : (ty : WasmType) -> FieldTypeCompat (Prim ty) (Prim ty)
  ||| Region references must refer to the same region (by name).
  RegRefCompat : FieldTypeCompat (RegRef name) (RegRef name)
  ||| Pointer kinds must match and pointed-to types must be compatible.
  PtrCompat : FieldTypeCompat inner1 inner2
           -> FieldTypeCompat (PtrType kind inner1) (PtrType kind inner2)
  ||| Optional wrapping is transparent for compatibility.
  OptCompat : FieldTypeCompat inner1 inner2
           -> FieldTypeCompat (Optional inner1) (Optional inner2)

||| Proof that a single field in the import matches the corresponding
||| field in the export: same name, same type, same offset, same size.
public export
data FieldMatch : (exported : Field name ty) -> (imported : Field name ty) -> Type where
  ||| Fields match if they have the same name (by type-level equality)
  ||| and the same type (by type-level equality). Since Field is indexed
  ||| by both name and ty, constructing FieldMatch proves both agree.
  MkFieldMatch : FieldMatch (MkField {name} {ty}) (MkField {name} {ty})
...
data SchemaMismatch : Type where
  ...
  MissingField : (fieldName : String) -> (importMod : ModuleId) -> SchemaMismatch
  ||| A field exists in both but with different types.
  TypeMismatch : (fieldName : String) -> (exportType : String) -> (importType : String) -> SchemaMismatch
