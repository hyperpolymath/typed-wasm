...
| Review-state | Paired producer sign-off received 2026-05-30 (https://github.com/hyperpolymath/ephapax/issues/165[ephapax#165], https://github.com/hyperpolymath/affinescript/issues/402[affinescript#402])
|===

== Tracking issues for `[review]` → `[accepted]` promotion

The 2026-05-30 paired producer reviews surfaced the following open questions; each has a tracking issue and resolution is a precondition for `[accepted]` promotion. None block landing the v0 codec behind cargo features (already done in PR #77).

* https://github.com/hyperpolymath/typed-wasm/issues/93[#93] — annotate v0 producer surface (unused `WasmType` / `PtrKind` / `cardinality` / `capabilities` codepoints as `reserved`).
* https://github.com/hyperpolymath/typed-wasm/issues/94[#94] — pin `WBool` wire width (4-byte canonical vs lenient mixed-width validation).
* https://github.com/hyperpolymath/typed-wasm/issues/95[#95] — proposal 0003 (`typedwasm.region-imports`) placeholder for cross-module region foreign keys (Open Question §5).
* https://github.com/hyperpolymath/typed-wasm/issues/96[#96] — `typedwasm.capability-grants` (L15-C per-call-site, v1.4.x follow-up).
* https://github.com/hyperpolymath/typed-wasm/issues/97[#97] — producer-readiness checklist appendix + canonical multi-section emit ordering.
* https://github.com/hyperpolymath/affinescript/issues/444[affinescript#444] — extract `Tw_section.{Encode,Decode}` (dedup `build_section` before two more sections land).
* https://github.com/hyperpolymath/ephapax/issues/221[ephapax#221] — surface `Ty::Borrow` → `OwnershipKind::SharedBorrow` / `ExclBorrow` at emission (producer-readiness precondition).
