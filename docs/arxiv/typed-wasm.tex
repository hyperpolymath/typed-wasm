...
\textbf{typed-wasm}, a type system with a checked 10-level progressive
type safety core, originally developed for database query languages,
...
erased before code generation, yielding zero runtime overhead for the
checked core. Our principal contribution is
...
the framework with two draft research levels: \emph{tropical cost-tracking}
(Level~11) and \emph{epistemic safety} (Level~12). These extensions are
described as formalisation targets in the repository, but they are not yet
part of the checked claim envelope or shipped toolchain.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{A progressive type safety framework for Wasm memory}
    ...
    lifetime safety, linearity), plus two draft extensions (tropical
    cost-tracking and epistemic safety) from the database domain to the
    memory domain. The current checked claim envelope is Levels~1--10.
  ...
  \item \textbf{Draft tropical cost-tracking and epistemic safety extensions}
    (Levels~11--12, Section~\ref{sec:levels}): two novel extensions beyond
    traditional type safety. Level~11 models bounded access cost via a
    tropical semiring. Level~12 applies epistemic modal logic to stale-state
    reasoning across module boundaries. In the current repository, both
    remain draft research extensions rather than checked release artefacts.

  \item \textbf{A formalisation in Idris~2 with QTT}
    (Section~\ref{sec:formal}): a checked L1--10 core with dependent types
    that prove safety properties at compile time, plus draft L11--12 files
    that outline the next research steps. The checked core erases to the
    same runtime code shape as hand-written Wasm.
\end{enumerate}
...
We conjecture the following soundness property for a future typed-wasm
artifact that checks all 12 levels:
...
For the checked L1--10 core, the Idris~2 formalisation checks these
properties via the totality checker: every function in the shipped ABI
layer is declared \texttt{total}, and the type checker rejects any case
split that does not cover all constructors. This is not a mechanised
proof of progress/preservation in the traditional sense, but it provides
strong evidence for the properties encoded in the dependent types. The
L11--L12 files are not yet part of the checked package, so their claims
remain research targets rather than release-grade evidence.
...
provides checked dependent-type-level evidence for properties~(a)--(f) in
the shipped L1--10 core, but does not yet connect to a formal Wasm
operational semantics. Properties~(g)--(h) are currently part of the
draft L11--L12 research story, not the checked release envelope.

\subsection{Tropical and Epistemic Foundations}

Level~11 rests on the tropical semiring $(\mathbb{N} \cup \{\infty\},
\min, +, \infty, 0)$. The repository contains a draft Idris~2 file for
this level, including candidate lemmas such as commutativity of $\oplus$
(\texttt{tropAddComm}), the identity of $\infty$ for $\oplus$
(\texttt{tropAddIdentity}), and the identity of $0$ for $\otimes$
(\texttt{tropMulIdentity}). Access paths are intended to compose via
$\otimes$, with a \texttt{CostBounded} witness capturing the total path
cost. This draft is not yet part of the checked package.
...
otherwise. The repository likewise contains a draft Idris~2 file for
freshness lemmas such as \texttt{writerKnowsFresh},
\texttt{freshOrStale}, and \texttt{syncRestoresFresh}. These files are
useful research scaffolds, but they are not yet packaged or claimable as
release-grade proof artefacts.
...
We evaluate typed-wasm along three axes: (1)~the benchmark surface that
currently exists in the repository, (2)~the runtime cost goal of the
checked core, and (3)~a qualitative comparison with alternative safety
mechanisms.

\subsection{Current Benchmark Surface}

The current repository does \emph{not} yet ship a full end-to-end
toolchain benchmark for all four example programs. Its benchmark surface
is narrower: a parser benchmark over the checked core's
\texttt{04-ecs-game.twasm} example. In a local rerun of the shipped
\texttt{benchmarks/parser-bench.mjs}, 200 successful parses completed in
244.63\,ms total, or 1.223\,ms per parse on average.

This is useful evidence that the parser surface is responsive on the
largest shipped example, but it should not be read as a full type-checker,
code-generator, or end-to-end toolchain benchmark. Multi-module schema
agreement and the ABI proof layer therefore remain qualitatively analysed
rather than quantitatively benchmarked in the current artifact.

\subsection{Runtime Cost}

For the checked L1--10 core, typed-wasm targets \textbf{zero runtime
overhead} by design. All type
...
indirection tables are inserted in the checked core path. Binary-size and
whole-pipeline overhead claims for the draft L11--L12 extensions would
require additional measurements that the repository does not yet provide.
...
\begin{itemize}
  ...
  \item typed-wasm is the only approach in this comparison that is
    simultaneously \emph{progressive} (adoptable at multiple levels) and
    \emph{formally proven for its checked core} (with dependent types and
    proof erasure).
...
We additionally exercised the parser and schema-level invariants using the
repository's offline ECHIDNA harness (Section~\ref{sec:implementation}).
The current harness checks four properties: parse determinism,
well-formed parse rate, schema reflexivity at the parser-AST level, and
effect declaration consistency. In the current shipped configuration, 100
offline iterations completed without a counterexample. This is useful
supporting evidence for the checked parser surface, but it is not a
substitute for mechanised soundness or full toolchain fuzzing.
...
schema. By applying a progressive type safety framework to memory
regions, typed-wasm provides a checked L1--10 core plus draft L11--L12
research extensions:
...
\begin{enumerate}
  ...
  \item \textbf{A checked 10-level core} from instruction validity
    through linearity, with formal proofs in Idris~2 and zero runtime
    overhead through proof erasure, plus draft L11--L12 extensions for
    tropical cost-tracking and epistemic safety.
...
\begin{table}[ht]
\centering
\caption{Level achievement status (checked v1.0 core plus draft L11--L12 extensions).}
\label{tab:levelstatus}
\begin{tabular}{@{}rlllll@{}}
\toprule
\textbf{Lv} & \textbf{Name} & \textbf{Proof} & \textbf{Runtime} & \textbf{Tests} & \textbf{Status} \\
\midrule
1  & Instruction     & Region.idr       & Parser      & Parser + E2E & E2E \\
2  & Region-binding  & TypedAccess.idr  & Schema      & Parser + E2E & E2E \\
3  & Type-compat.    & TypedAccess.idr  & Load/store  & Parser + E2E & E2E \\
4  & Null safety     & Pointer.idr      & Ptr kinds   & Parser + E2E & E2E \\
5  & Bounds-proof    & Levels.idr       & Bounds chk  & Parser + E2E & E2E \\
6  & Result-type     & TypedAccess.idr  & Type flow   & Parser + E2E & E2E \\
\midrule
7  & Aliasing        & Pointer.idr      & Erased      & Proof + examples & Proven [sfap] \\
8  & Effects         & Effects.idr      & Erased      & Proof + examples & Proven [sfap] \\
9  & Lifetime        & Lifetime.idr     & Erased      & Proof + examples & Proven [sfap] \\
10 & Linearity       & Linear.idr       & Erased      & Proof + examples & Proven [sfap] \\
\midrule
11 & Tropical cost   & Tropical.idr     & ---         & ---    & Draft only; not in package \\
12 & Epistemic       & Epistemic.idr    & ---         & ---    & Draft only; not in package \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{``Draft'' (L11--12).}
Levels~11 and 12 currently have Idris~2 draft files and proof structures
with zero \texttt{believe\_me}, but they are not part of the checked
package and are not yet integrated into the toolchain. They should be
treated as research extensions rather than shipped proof artefacts.
...
\texttt{assert\_total}. The checked release envelope, however, currently
consists only of the shipped L1--10 package; Tropical and Epistemic are
draft files outside that envelope.
