...
foundation + wasm e2e round-trip). Ephapax side: adopt-when-implemented —
this document is the contract its async lowering must meet (no divergence:
the closure ABI already converges byte-identically). AffineScript #225
PR 2–4 (the CPS transform proper) are unblocked.
