...
// ----------------------------------------------------------------------
// L13 region-imports carrier — `typedwasm.region-imports` custom section
//
// Pre-staged against typed-wasm proposal 0003 (typed-wasm#140, refs #95).
// UNSTABLE: wire format may change before the proposal is [accepted].
//
// Wire format (little-endian fixed-width fields, unsigned LEB128 varints,
// byte-aligned, lenient on truncation):
//
//   u16le         version              (= REGION_IMPORTS_SECTION_VERSION = 1)
...
//       u8[]          producer_module_name      (UTF-8)
//       u32le_leb128  region_name_len
//       u8[]          region_name               (UTF-8)
//       u32le_leb128  expected_field_count
//       for each expected field (in declaration order):
//           u32le_leb128  field_name_len
//           u8[]          field_name             (UTF-8)
//           u8            kind                   (FieldKind enum;
//                                                  pointer kinds rejected
//                                                  in v1 — proposal 0003
//                                                  §Producer obligations 5)
//           u8            wasm_ty                (WasmTy enum)
//           u8            nullability            (Nullability enum)
//           u32le_leb128  cardinality
//
// Companion to typedwasm.regions's `target_region` value-space extension:
// values `>= IMPORT_TABLE_BASE` (and != NO_TARGET_REGION) in regions field
// entries resolve through this section's import table — index =
// `value - IMPORT_TABLE_BASE`.
//
// L13 cross-module schema agreement (the `SchemaSub expected actual`
// witness from MultiModule.idr) defers to a later `verify_link_graph`
// pass — this section's verifier pass only checks in-module
// self-consistency (proposal 0003 §"Open questions" #4 default option a).
// ----------------------------------------------------------------------

#[cfg(feature = "unstable-l13-imports")]
pub const REGION_IMPORTS_SECTION_VERSION: u16 = 1;

/// High-bit threshold for the cross-section `target_region` extension
/// described in proposal 0003 §"Cross-section foreign-key extension":
/// `typedwasm.regions` field entries with `target_region >= IMPORT_TABLE_BASE`
/// (and `!= NO_TARGET_REGION`) resolve through the region-imports table.
#[cfg(feature = "unstable-l13-imports")]
pub const IMPORT_TABLE_BASE: u32 = 0x8000_0000;

/// One expected-field record inside an imported region's schema. Mirrors
/// the on-wire shape and the `Field` constructor at `Region.idr:127`.
/// Pointer kinds (`kind != Scalar`) are rejected at verify time in v1.
#[cfg(feature = "unstable-l13-imports")]
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ImportedFieldEntry {
    pub name: String,
    pub kind: FieldKind,
    pub wasm_ty: WasmTy,
    pub nullability: Nullability,
...
/// One import-table entry — names the producer module, the exported
/// region name in that module, and the schema the importer expects.
/// Mirrors `ImportedRegion` at `MultiModule.idr:53`.
#[cfg(feature = "unstable-l13-imports")]
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RegionImportEntry {
    pub producer_module_name: String,
    pub region_name: String,
    pub expected_fields: Vec<ImportedFieldEntry>,
}

/// Parse the `typedwasm.region-imports` custom-section payload. Lenient
/// on truncation (matches sibling carriers): a short read fills missing
/// bytes with zero, which decodes to NonNull / cardinality=0 / etc.
/// Returns `None` if `version != REGION_IMPORTS_SECTION_VERSION` — future
/// breaking bumps belong in a new public API.
#[cfg(feature = "unstable-l13-imports")]
pub fn parse_region_imports_section_payload(payload: &[u8]) -> Option<Vec<RegionImportEntry>> {
    ...
    let mut imports = Vec::with_capacity(import_count.min(1_048_576) as usize);
    for _ in 0..import_count {
        let producer_module_name = read_utf8_leb128(&mut r);
        let region_name = read_utf8_leb128(&mut r);
        let field_count = read_u32_leb128(&mut r);
        let mut expected_fields = Vec::with_capacity(field_count.min(65_536) as usize);
        for _ in 0..field_count {
            let name = read_utf8_leb128(&mut r);
            let kind = FieldKind::from_byte(r.read_u8());
            let wasm_ty = WasmTy::from_byte(r.read_u8());
            let nullability = Nullability::from_byte(r.read_u8());
            let cardinality = read_u32_leb128(&mut r);
            expected_fields.push(ImportedFieldEntry {
                name,
                kind,
                wasm_ty,
                nullability,
...
/// Encode region-import entries.
/// `parse(build(x)) == Some(x)` for any valid input.
#[cfg(feature = "unstable-l13-imports")]
pub fn build_region_imports_section_payload(imports: &[RegionImportEntry]) -> Vec<u8> {
    let mut out = Vec::with_capacity(2 + 5 + imports.len() * 32);
    ...
    for imp in imports {
        write_utf8_leb128(&mut out, &imp.producer_module_name);
        write_utf8_leb128(&mut out, &imp.region_name);
        ...
        for f in &imp.expected_fields {
            write_utf8_leb128(&mut out, &f.name);
...
// UTF-8 string with an LEB128 length prefix. Proposal 0003 uses LEB128
// throughout (matches access-sites in proposal 0002); the existing
// `read_utf8`/`write_utf8` helpers above use u32le and serve proposals
// 0001/0004, so the LEB128 variants below are distinct.
#[cfg(feature = "unstable-l13-imports")]
fn read_utf8_leb128(r: &mut LenientReader<'_>) -> String {
    let len = read_u32_leb128(r) as usize;
    let cap = len.min(65_536);
...
#[cfg(feature = "unstable-l13-imports")]
fn write_utf8_leb128(out: &mut Vec<u8>, s: &str) {
    let bytes = s.as_bytes();
    let len: u32 = bytes.len().try_into().expect("name length must fit in u32");
...
mod region_imports_tests {
    ...
    fn scalar_import_field(name: &str, ty: WasmTy, cardinality: u32) -> ImportedFieldEntry {
        ImportedFieldEntry {
            name: name.to_string(),
            kind: FieldKind::Scalar,
            wasm_ty: ty,
    ...
    #[test]
    fn version_only_yields_zero_imports() {
        let mut payload = REGION_IMPORTS_SECTION_VERSION.to_le_bytes().to_vec();
        payload.push(0); // LEB128 0 = import_count
        assert_eq!(parse_region_imports_section_payload(&payload), Some(vec![]));
    }

    #[test]
    fn wrong_version_returns_none() {
        let payload = [99u8, 0, 0];
        assert_eq!(parse_region_imports_section_payload(&payload), None);
    }

    #[test]
    fn roundtrip_single_import_no_fields() {
        let imports = vec![RegionImportEntry {
            producer_module_name: "world".to_string(),
            region_name: "Player".to_string(),
            expected_fields: vec![],
        }];
        let bytes = build_region_imports_section_payload(&imports);
        assert_eq!(parse_region_imports_section_payload(&bytes), Some(imports));
    }

    #[test]
    fn roundtrip_single_import_with_scalar_fields() {
        let imports = vec![RegionImportEntry {
            producer_module_name: "world".to_string(),
            region_name: "Player".to_string(),
            expected_fields: vec![
                scalar_import_field("hp", WasmTy::I32, 1),
                scalar_import_field("speed", WasmTy::F64, 1),
                scalar_import_field("name", WasmTy::U8, 16),
            ],
        }];
        let bytes = build_region_imports_section_payload(&imports);
        assert_eq!(parse_region_imports_section_payload(&bytes), Some(imports));
    ...
    #[test]
    fn roundtrip_multiple_imports_same_producer() {
        let imports = vec![
            RegionImportEntry {
                producer_module_name: "world".to_string(),
                region_name: "Player".to_string(),
                expected_fields: vec![scalar_import_field("hp", WasmTy::I32, 1)],
            },
            RegionImportEntry {
                producer_module_name: "world".to_string(),
    ...
    #[test]
    fn roundtrip_pointer_kinds_codec_only() {
        // The codec round-trips pointer kinds faithfully; the verifier
        // pass rejects them in v1 (proposal 0003 §Producer obligations 5).
        // This test pins the codec's tolerance.
        let imports = vec![RegionImportEntry {
            producer_module_name: "world".to_string(),
            region_name: "Container".to_string(),
            expected_fields: vec![ImportedFieldEntry {
    ...
    fn truncated_payload_zero_fills_trailing_fields() {
        let imports = vec![RegionImportEntry {
            producer_module_name: "w".to_string(),
            region_name: "R".to_string(),
            expected_fields: vec![scalar_import_field("f", WasmTy::I32, 7)],
        }];
        let full = build_region_imports_section_payload(&imports);
        // Chop the trailing cardinality LEB128 (single byte for value 7).
        let truncated = &full[..full.len() - 1];
        let parsed = parse_region_imports_section_payload(truncated).expect("parses leniently");
        // cardinality missing → reads as 0
        assert_eq!(parsed[0].expected_fields[0].cardinality, 0);
    }

    #[test]
    fn utf8_lossy_recovers_bad_bytes() {
        // Build a payload with an invalid UTF-8 byte in the producer name.
        let mut payload = REGION_IMPORTS_SECTION_VERSION.to_le_bytes().to_vec();
        payload.push(1); // import_count = 1
        payload.push(2); // producer_module_name_len = 2
        payload.push(0xC0); // invalid UTF-8 start byte
        payload.push(0xC0);
        payload.push(0); // region_name_len = 0
        payload.push(0); // expected_field_count = 0
        let parsed = parse_region_imports_section_payload(&payload).expect("parses leniently");
        // Two REPLACEMENT CHARACTERs (U+FFFD), each encoding to 3 UTF-8 bytes.
        assert_eq!(parsed[0].producer_module_name, "\u{FFFD}\u{FFFD}");
    }

    #[test]
    fn leb128_handles_large_field_counts() {
        let imports = vec![RegionImportEntry {
            producer_module_name: "world".to_string(),
            region_name: "Big".to_string(),
            expected_fields: (0..200u32)
                .map(|i| scalar_import_field(&format!("f{i}"), WasmTy::U32, 1))
                .collect(),
        }];
        let bytes = build_region_imports_section_payload(&imports);
        assert_eq!(parse_region_imports_section_payload(&bytes), Some(imports));
