...
out). The `+wasmi-runtime+` feature (default) ships a pure-Rust
in-process adapter that CI executes end-to-end; a *wasmtime adapter* is
the intended follow-up and needs nothing beyond what the wasmi one uses
